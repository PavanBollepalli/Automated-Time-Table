...
import { getTimetables, getTimetable, deleteTimetable, aiAnalyze, getScheduleConfigBySemester, type ScheduleConfig } from "@/lib/api";
import TimetableGrid from "@/components/TimetableGrid";
import { Loader2, Calendar, Eye, X, Trash2, Sparkles } from "lucide-react";
import { toast } from "@/components/ui/use-toast";

interface TimetableListViewProps {
  /** Allow the user to delete timetables */
  canDelete?: boolean;
  /** Allow the user to run AI analysis */
  canAnalyze?: boolean;
}
...
export default function TimetableListView({
  ...
  const viewDetail = async (id: string) => {
    ...
    try {
      ...
      setSelectedTT(tt);
      // Try to fetch schedule config for this timetable's semester
      if (tt.semester_id) {
        try {
          const cfg = await getScheduleConfigBySemester(tt.semester_id);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.detail || err?.message || "Failed to delete timetable";
            ...
            <div className="flex gap-1">
              {canAnalyze && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleAnalyze(selectedTT.id)}
                  disabled={analyzing}
                >
                  {analyzing ? (
          ...
          <CardContent>
            <div className="flex flex-wrap gap-4 mb-4 text-sm">
              <div>
                ...
                <span className="font-medium">{selectedTT.program_name || "—"}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Batch:</span>{" "}
                <span className="font-medium">{selectedTT.batch_name || "—"}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Semester:</span>{" "}
                <span className="font-medium">{selectedTT.semester_name || "—"}</span>
              </div>
              {selectedTT.section_name && (
                <div>
                  <span className="text-muted-foreground">Section:</span>{" "}
                  <span className="font-medium">{selectedTT.section_name}</span>
                </div>
              )}
              <div>
                <span className="text-muted-foreground">Entries:</span>{" "}
                <span className="font-medium">{selectedTT.entries?.length || 0}</span>
              </div>
              {selectedTT.is_draft && (
            ...
            {selectedTT.entries?.length > 0 ? (
              <TimetableGrid entries={selectedTT.entries} scheduleConfig={scheduleConfig} />
            ) : (
              <p className="text-sm text-muted-foreground text-center py-8">
                No entries in this timetable
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Timetables will appear here once generated
            </p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Batch: {t.batch_name || "—"}
                      {t.section_name ? ` · Section ${t.section_name}` : ""}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Semester: {t.semester_name || "—"}
                    </p>
                  </div>
                  {t.is_draft && (
                    <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full">
                ...
                <div className="mt-3 flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">
                    {t.entries?.length || 0} entries
                  </span>
                  <div className="flex gap-1">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => viewDetail(t.id || t._id)}
