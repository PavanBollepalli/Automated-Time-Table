...
export default function DeoTimetablesPage() {
  ...
  useEffect(() => { getTimetables().then(setTimetables).catch(() => {}).finally(() => setLoading(false)); }, []);

  const viewDetail = async (id: string) => {
    setLoadingDetail(true); try { setSelectedTT(await getTimetable(id)); } catch {} setLoadingDetail(false);
  };
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-bold tracking-tight">Timetables</h1><p className="text-muted-foreground mt-1">View all generated timetables</p></div>
      {selectedTT && (
        <Card><CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle>Timetable Details</CardTitle><Button variant="ghost" size="sm" onClick={()=>setSelectedTT(null)}><X className="h-4 w-4"/></Button></CardHeader><CardContent>
      ...
      {timetables.length===0?(
        <Card className="border-dashed"><CardContent className="flex flex-col items-center justify-center py-12 text-center">
          <Calendar className="h-12 w-12 text-muted-foreground/50 mb-4" /><h3 className="text-lg font-semibold">No timetables yet</h3>
        </CardContent></Card>
      ):(
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">{timetables.map((t:any)=>(
          <Card key={t.id||t._id} className="hover:shadow-md transition-shadow"><CardContent className="pt-6">
            ...
            <p className="text-sm text-muted-foreground">Batch: {t.batch||"—"} | Sem: {t.semester||"—"}</p>
